"""Tests for the Survival active-exit paper-test PowerShell runbook."""
...
SCRIPT = ROOT / "tools" / "run_survival_active_paper_test.ps1"
...
def test_survival_active_runbook_parser_accepts_required_minutes():
    text = _script_text()
...
def test_survival_active_runbook_uses_mode_manager_and_active_override():
    text = _script_text()
    ...
    for snippet in (
        ...
        "$forcedPaperEnv['SURVIVAL_EXIT_RISK_THRESHOLD']",
        "$forcedPaperEnv['EXEC_RESTORE_STATE'] = 'false'",
        "survival_active_mode_env.json",
        "tools\\verify_survival_exit.py",
        "survival_active_paper_summary.json",
    ):
...
def test_survival_shadow_mode_remains_placeholder_safe():
    env = get_experiment_mode("survival_shadow").overrides
    placeholder = get_experiment_mode("survival_active_placeholder").overrides

    assert env["USE_SURVIVAL_EXIT"] == "true"
    assert env["SURVIVAL_EXIT_ACTIVE"] == "false"
    assert placeholder["USE_SURVIVAL_EXIT"] == "true"
    assert placeholder["SURVIVAL_EXIT_ACTIVE"] == "false"


def test_survival_active_runbook_refuses_live_or_real_order_mode():
    text = _script_text()
...
def test_survival_active_runbook_starts_executor_in_paper_mode_only():
    text = _script_text()
...
def test_survival_active_runbook_parses_when_powershell_available():
